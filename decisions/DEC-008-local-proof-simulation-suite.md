...
**Re-verification (2026-07-03):** the full VM-backed trilogy was re-run green on the same substrate after the eBPF/agent changes of that week — **L2b** (real CO-RE agent loads on the BTF kernel and classifies cross_az on ~90 MB of real captured traffic, landing on the backend-node agent per DEC-010), **the differential** (unchanged verdict: Tollwing `$0.001994` to the dialed Service, OpenCost `$0`, Kubecost `same_zone=free` source-pod-only), and **L3** (agent held **5 MiB** heap under 4-client sustained load, goroutines 49→49, 0 restarts — well inside the P2 budget). The tiers are now one-command repeatable via `make sim-vm`/`sim-l2b`/`sim-differential`/`sim-l3` (the kind node image is pinned and the BTF VM is auto-created); the kubeadm bring-up is occasionally flaky on Apple-silicon Lima and may need a retry.
